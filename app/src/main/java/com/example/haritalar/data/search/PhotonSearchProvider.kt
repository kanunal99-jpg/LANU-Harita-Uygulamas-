 ...
 * Powered by OpenStreetMap and Elasticsearch, providing fast autocomplete,
 * fuzzy matching, and typo-tolerant search across Turkey.
 */
class PhotonSearchProvider(
    ...
    override suspend fun search(query: String, focusPoint: GeoPoint?): List<SearchResult> = withContext(Dispatchers.IO) {
        ...
        val urlBuilder = StringBuilder("https://photon.komoot.io/api/?q=").append(encodedQuery)
            .append("&limit=15")
        ...
        if (!response.isSuccessful) {
            throw IOException("Photon HTTP Error: ${response.code} ${response.message}")
        }
        ...
        for (i in 0 until features.length()) {
            ...
            // Filter results to Turkey if country code is present
            ...
            val resultType = determineResultType(osmKey, osmValue, details)
    ...
    override suspend fun reverseGeocode(point: GeoPoint): String? = withContext(Dispatchers.IO) {
        try {
            ...
            val props = first.optJSONObject("properties") ?: return@withContext null
